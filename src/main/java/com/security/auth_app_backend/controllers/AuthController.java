...
import com.security.auth_app_backend.dtos.LoginRequest;
import com.security.auth_app_backend.dtos.RefreshTokenRequest;
import com.security.auth_app_backend.dtos.TokenResponse;
...
import com.security.auth_app_backend.security.CookieService;
import com.security.auth_app_backend.security.JwtService;
...
public class AuthController {
    ...
    public ResponseEntity<TokenResponse> login(
        ...
        .jti(jti).user(user).createdAt(Instant.now()).expiresAt(Instant.now().plusSeconds(jwtService.getRefreshTtlSeconds())).revoked(false).build();
    ...
    private Optional<String> readRefreshTokenFromRequest(RefreshTokenRequest body, HttpServletRequest request) {
        if (request.getCookies()!=null) {
            Optional<String> fromCookie = Arrays.stream(request.getCookies()).filter(c->cookieService.getRefreshTokenCookieName().equals(c.getName())).map(c->c.getValue()).filter(v->!v.isBlank()).findFirst();
